...
client.once("ready", async () => {
  console.log(`started ${client.user!.tag} with ${client.interactions.modules.size} interaction(s) cached`)

  const cmds: ApplicationCommandDataResolvable[] =
    <any[]>client.interactions.modules.map((m: any) =>
      ({ name: m.name, type: m.type, description: m.description, options: m.options, defaultPermission: m.defaultPermission })
    );
...
client.interactions.on("error", (e, i, m) => {
  console.log(`${m.name} (${m.type}) ${i.user.tag} got an error :: ${e.message}`)
})
