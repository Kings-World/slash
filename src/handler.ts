import { AkairoClient, AkairoHandler, AkairoHandlerOptions } from "@kingsworld/akairo"
import { Interaction as Base, InteractionOptions } from "./class"
import type { AutocompleteInteraction, CommandInteraction, ContextMenuInteraction, Interaction } from "discord.js";
...
export class InteractionHandler extends AkairoHandler {
    ...
    this.client.once("ready", () => {
      ...
      this.client.on("interactionCreate", (interaction: Interaction) => {
        ...
        if (interaction.isAutocomplete()) {
          ...
          return handle(this.modules.get(interaction.commandName), focused, true)
        }
  ...
  parseFunc(interaction: Interaction) {
    return async (module: any, options?: boolean | any, ac?: boolean) => {
      if (!module) return;
      options = options === true ? this.transform(<any>interaction) : options ?? {};

      try {
        this.emit("started", interaction, module, options)
        return await module?.[ac ? "autocomplete" : "exec"](interaction, options)
      } catch (e) {
  ...
  transform(interaction: CommandInteraction | ContextMenuInteraction | AutocompleteInteraction, options?: InteractionOptions["options"]) {
    ...
    return Object.assign(
      (options ?? []).reduce((o, i) => ({ ...o, [i.name]: toDefault(i.default) }), {}),
      ...mappedOptions
    )
  }
